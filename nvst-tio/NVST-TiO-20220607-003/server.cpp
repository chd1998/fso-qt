...
void Server::ack(int retcode) {
    ...
    const char* msg_data;
    QString qmsg_data=QString::number(retcode);
    msg_data=qmsg_data.toLatin1().data();
...
Request Server::parseRequest(const char *requestData) {
    Request req;
    QStringList itemList = QString().fromStdString(requestData).split(';', Qt::SkipEmptyParts);
    for (QString &item: itemList) {
        ...
        if (kv[0] == 'i') {
            ...
            QStringList pList;
            if (kv[1].contains(',')) {
                pList = kv[1].split(',', Qt::SkipEmptyParts);
            } else {
                pList.append(kv[1]);
            }
