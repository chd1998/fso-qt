...
void MainWindow::on_btnSnap_pressed() {
    ...
    if(!savefits  && live && opened && !fulldisk && andorCCD->isRunning())
    {
        localfirst=true;
        QDateTime current_date_time =QDateTime::currentDateTimeUtc();
...
void MainWindow::drawHist(QImage grayimage)
{
    qDebug("here inside...");
   ...
   qDebug()<<max<<" "<<idx;
    ...
    {
        tmpstring="0";
        axisX->append("<font color=\"red\">"+tmpstring+"</font>", 1);
        axisX->append(QString::number(65535),255);
    }else{
        if(idx==255)
        {
            axisX->append(QString::number(0),1);
            tmpstring = QString::number(65535);
            axisX->append("<font color=\"red\">"+tmpstring+"</font>", idx);
        }else{
            axisX->append(QString::number(0),0);
            tmpstring = QString::number(idx*65535/255);
            axisX->append("<font color=\"red\">"+tmpstring+"</font>", idx);
            axisX->append(QString::number(65535),255);
        }
    }
...
void MainWindow::updateGraphicsView(unsigned short* buf,uint buflen) {
    if(live && NULL != buf && andorCCD->isRunning())
    {
        QImage *qimage;
