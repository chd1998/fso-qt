...
MainWindow::MainWindow(QWidget *parent): QMainWindow(parent), ui(new Ui::MainWindow) {
    ...
    if(realDev>=1)
    {
        ...
        if(andorCCD->isRunning())
        {
            ...
            connect(andorCCD,&aCCD::buf_Ready,andorCCD,&aCCD::saveData,Qt::DirectConnection);

            //connect(andorCCD,&aCCD::buf_Ready,this,&MainWindow::updateGraphicsView,Qt::DirectConnection);
            //connect(andorCCD,&aCCD::buf_Ready,task,&histdisplay::buf2hist,Qt::DirectConnection);
            //connect(task,&histdisplay::draw_hist,this,&MainWindow::drawHist,Qt::BlockingQueuedConnection);
...
void MainWindow::on_btnLive_pressed() {
    if (!live && opened) {
        ...
        /*disconnect(andorCCD,&aCCD::buf_Ready,this,&MainWindow::updateGraphicsView);
        disconnect(andorCCD,&aCCD::buf_Ready,task,&histdisplay::buf2hist);
        disconnect(task,&histdisplay::draw_hist,this,&MainWindow::drawHist);*/
...
void MainWindow::on_btnSnap_pressed() {
    ...
    }else
    {
        ...
        //disconnect(andorCCD,&aCCD::buf_Ready,andorCCD,&aCCD::saveData);
    }
}
...
void MainWindow::updateGraphicsView(unsigned short* buf,uint buflen,QString savePath,uint count_num) {
    if(live)
    {
        QMutex displock;
        displock.lock();
...
void MainWindow::on_textEdit_status_textChanged()
{
    ui->textEdit_status->moveCursor(QTextCursor::End, QTextCursor::MoveAnchor);
}
