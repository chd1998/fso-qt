...
void aCCD::getData()
{
    ...
    if(savefits && !diskfull  )
    {
        ...
        if(fpre=="dark")
        {
            ...
            saveDir=savepred+"\\"+current_date_t1+"\\"+current_date_t1;
            if(!sdir.exists(saveDir))
                sdir.mkpath(saveDir);
            //acqcnt=1000;
        }
    ...
    for (int i=0; i < NumberOfFrames; i++)
    {
        ...
        if(localfirst)
        {
         ...
         //if(live && !display_locked && display && i>=1)
         if(live && i>=1)
             emit buf_Ready(unpackedBufferback,buflen,saveDir,i);
...
void aCCD::saveData(unsigned short* buff,uint buflen,QString savePath,uint count_num)
{
    if(savefits && !localfirst)
    {
        QString fstr;
