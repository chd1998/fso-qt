...
void MainWindow::drawHist(cv::Mat grayimage)
{
    qDebug("here inside...");
       ...
       //qDebug()<<max;
        //设置每个“柱”的颜色，值，宽度等
        QBarSet *set = new QBarSet("Pixels");
        for (int i=0;i<256;i++) {
           set->append(data[i]);
...
void MainWindow::updateGraphicsView(unsigned short* buf,uint buflen) {
    if(live && NULL != buf)
    {
        ...
        if(lossyImage.data && histready)
        {
            histready=false;
            emit histReady(lossyImage);
        }
        ...
        qimage= new QImage(static_cast<unsigned char *>(lossyImage.data), lossyImage.cols, lossyImage.rows, static_cast<int>(lossyImage.step), QImage::Format_Grayscale8);
        scene->clear();
