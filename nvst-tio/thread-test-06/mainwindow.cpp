...
void MainWindow::on_btn_start_A_clicked()
{
    ...
    if(((imgX != imgX0) || (imgY != imgY0)) && ((imgX % 2) ==0 && (imgY % 2) ==0))
    {
    ...
    }else
    {
        imgX=1024;
        imgY=1024;
        ui->textEdit_StatusA->append("Image Width and Height Must Be 2^n");
        ui->textEdit_StatusA->append("Using Default Value now...");
        ui->lineEdit_imgx->setText(QString::number(imgX));
        ui->lineEdit_imgy->setText(QString::number(imgY));
    }
...
void MainWindow::updateImg()
{
    ...
    if((!pausedA || !stoppedA) && startedA)
    {
        ...
        *grayimage=grayimage16->convertToFormat(QImage::Format_Indexed8);
        ...
        //ui->imgView->fitInView(imgscene->sceneRect(),Qt::IgnoreAspectRatio);
        ui->imgView->update();
    }
    imglock.unlock();
    imglocked = false;
