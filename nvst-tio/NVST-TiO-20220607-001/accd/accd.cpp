...
void aCCD::getData()
{
    ...
    unpackedBufferback01=unpackedBuffer;
    ...
    for (int i=0; i < NumberOfFrames; i++) {
         ...
         if(NULL != unpackedBufferback)
         {
            ...
            if(display && !display_locked)
                emit buf_Ready(unpackedBufferback);
            if(!histcalc_locked && !histshow_locked)
            {
                QVector<unsigned short>vecimg(unpackedBufferback01,unpackedBufferback01+imgW*imgH);
