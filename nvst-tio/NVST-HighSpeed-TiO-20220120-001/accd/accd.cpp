...
void aCCD::run()
{
    ...
    while (!isInterruptionRequested())
    {
        if(live)
        {
            ...
            dt=lt2-lt1;
            if(fpre=="T" && dt>=30 && (serialNo % 200)==0)
            {
                localsave=true;
                lt1=lt2;
            }
...
=======
            QDir tmpdir;
>>>>>>> 199f7925c8920ab2900a0965bb04870a9844f24e
            QStringList filter;
            filter<<"*.fits";
...
>>>>>>> 199f7925c8920ab2900a0965bb04870a9844f24e
    ...
    //QDir dir;
    //saveDir=savepre;
    if(savefits && !saveDir.isEmpty() && !sdir.exists(saveDir) && !fulldisk)
    {
    ...
    /*if(savefits && !saveDir.isEmpty() && localsave)
    {
        qDebug()<<"Saving Pre: "<<savepred<<" "<<savepref<<" "<<savepre;
        qDebug()<<"Saving Dir: "<<saveDir;
    }*/
    ...
    for (int i=0; i < NumberOfFrames; i++) {
         ...
         //imgMax=*std::max_element(unpackedBufferback,unpackedBufferback+sizeof(unpackedBufferback)/sizeof(unpackedBufferback[0]));
         //imgMax=*std::max_element(std::begin(unpackedBufferback), std::end(unpackedBufferback));
...
int aCCD::saveData(QString savePath,unsigned short* buff)
{
    ...
    if(status == 0)
    {
        ...
        QDateTime current_date_time =QDateTime::currentDateTimeUtc();
        current_date_d =current_date_time.toString("yyyyMMdd");
        current_date_t =current_date_time.toString("hhmmss");
        QString obsdate=current_date_d+"T"+current_date_t;
