...
float expTime;
int frameRate,crossX=0,crossY=0;
QString saveTo,ccdM,fpre="NVST-Dark";
quint16 imgMax=0;
bool savefits=false,drawing=false,nightmode=false;
double temperature=0,sx=0,sy=0,ex=0,ey=0;
...
void MainWindow::setupStatusBar() {
    ...
    labelpicinfo = new QLabel(this);
    labelpicinfo->setText("Image info");
    labelpicinfo->setAlignment(Qt::AlignCenter);
    labelpicinfo->setMinimumWidth(60);

    ui->statusBar->addWidget(labelpicinfo);
}
...
void MainWindow::on_btnLive_toggled(bool checked) {
    ...
    if (checked) {
        ...
        labelpicinfo->setText(QString::number(imageHeight)+"x"+QString::number(imageWidth)+" 16bits");
    } else {
        liveTimer->stop();
        delete liveTimer;
        liveTimer = nullptr;
...
void MainWindow::updateGraphicsView() {
    ...
    QString hrs=ctime.toString("H");
    qDebug()<<hrs.toInt();
    if(hrs.toInt()>=7 && hrs.toInt()<=19)
        nightmode=false;
    else
        nightmode=true;
    ...
    if(drawing)
    {
        if(nightmode)
        {
            QString position=QString::number(crossX)+","+QString::number(crossY);
            drawMarker(lossyImage,cv::Point(crossX,crossY),cv::Scalar(255,225,255),cv::MarkerTypes::MARKER_TILTED_CROSS,100,2,8);
            putText(lossyImage,position.toStdString(),cv::Point(crossX+40,crossY+15),cv::FONT_HERSHEY_COMPLEX,2,cv::Scalar(255,255,255),2,8,0);
        }else{
