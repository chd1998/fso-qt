...
void MainWindow::updateGraphicsView(unsigned short* buf,uint buflen) {
    ...
    if(live  && NULL != buf)
    {
        ...
        if(nightmode && live)
        {
            ...
            cv::drawMarker(lossyImage,cv::Point(crossX,crossY),cv::Scalar(255,225,255),cv::MarkerTypes::MARKER_TILTED_CROSS,100,2,8);
            cv::putText(lossyImage,position.toStdString(),cv::Point(crossX+40,crossY+15),cv::FONT_HERSHEY_COMPLEX,2,cv::Scalar(255,255,255),2,8,0);
        }

        if(drawing && live && !nightmode)
        {
