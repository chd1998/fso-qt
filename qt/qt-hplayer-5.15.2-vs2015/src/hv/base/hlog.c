...
void logger_set_file(logger_t* logger, const char* filepath) {
    //strncpy_s(logger->filepath, ,filepath, sizeof(logger->filepath));
    strncpy_s(logger->filepath, sizeof(logger->filepath),filepath,sizeof(logger->filepath));
    // remove suffix .log
...
static FILE* logfile_shift(logger_t* logger) {
    ...
    if (logger->fp_ == NULL) {
        ...
        errno=fopen_s(&logger->fp_,logger->cur_logfile,"a");
    ...
    if (logger->fp_ && --logger->can_write_cnt < 0) {
        ...
        if (filesize > logger->max_filesize) {
            ...
            errno=fopen_s(&logger->fp_,logger->cur_logfile, "w");
