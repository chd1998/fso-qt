...
QString NetTxt="";
QString SPName="/dev/ttyUSB0",SPParityBits="none",siteID="Site-01",gpsIP="*";
    ...
    ui(new Ui::MainWindow)
{
    ...
    this->setWindowTitle("GPS for Site Survey (Ver. 0.1.2 - MultiThread - linux)");
    ...
    gps01->startServer();
    displayMsg("Listening on port "+QString::number(gpsIPPort));
    //connect(gps01,&MyServer::sendMsg2UI,this,&MainWindow::displayMsg);
    //MyServer gps01;
    //gps01.startServer();
...
{
    QFile cfgfile(QDir::currentPath() + "//gps_site.json");
...
bool MainWindow::openCfg()
{
    //打开Json文件并用QByteArray读取全部内容
        ...
        QFile file(QDir::currentPath() + "//gps_site.json");
...
void MainWindow::readSPData()//创建一个readData对象
{
    if(opened)
    {
        ...
        ui->comInformationTextEdit->append(siteID+","+lat_str+","+lon_str+","+alt_str+","+time_str+","+date_str);
        GPS_list.clear();
        GPS_list_2.clear();
        GPS_Data.clear();
        GPS_Data_2.clear();
