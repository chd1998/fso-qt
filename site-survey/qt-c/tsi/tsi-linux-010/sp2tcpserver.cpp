...
void SP2TCPServer::run()
{
    ...
    while(!isInterruptionRequested())
    {
        ...
        emit sendMsg2UI(QString::number(idx)+" - "+clientIP+" - "+current_date+","+current_time+","+devID+","+QString::number(recvTSI));
        msleep(1000);
        idx++;
    }
}
...
void SP2TCPServer::sendData()
{
    ...
    gClientConnection->write(current_date.toLatin1()+","+current_time.toLatin1()+","+devID.toLatin1()+","+recvTSI_str.toLatin1());
    gClientConnection->disconnectFromHost();
    QString tmpMsg;
    tmpMsg=clientIP+" - "+current_date+","+current_time+","+devID+","+QString::number(recvTSI);
